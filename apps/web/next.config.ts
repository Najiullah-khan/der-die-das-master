...
// statically served /word/[slug] SEO pages at ~zero compute). 'unsafe-inline' on script/style is
// the documented fallback for exactly this case (Next's hydration payload and our JSON-LD
// <script> tags both need inline execution with no nonce available on a static page).
const cspHeader = `
  default-src 'self';
...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [{ source: "/codex", destination: "/dictionary", permanent: true }];
  },
};
